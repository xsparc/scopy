...
void SpectrumAnalyzer::initInstrumentStrings()
{
	...
	mag_types = {
		{tr("dBFS"), FftDisplayPlot::DBFS}, {tr("dBV"), FftDisplayPlot::DBV},
		{tr("dBu"), FftDisplayPlot::DBU},   {tr("Vpeak"), FftDisplayPlot::VPEAK},
		{tr("Vrms"), FftDisplayPlot::VRMS}, {tr("V/√Hz"), FftDisplayPlot::VROOTHZ},
	};
...
SpectrumAnalyzer::SpectrumAnalyzer(struct iio_context *ctx, Filter *filt, ToolMenuEntry *tme, m2k_iio_manager *m2k_man,
	...
	, bin_sizes({256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144})
	, nb_ref_channels(0)
	...
	, use_float_sink(true)
{
...
#ifdef SPECTRAL_MSR
	connect(fft_plot, SIGNAL(channelAdded(int)), this, SLOT(onChannelAdded(int)));
	...
	for(int i = 0; i < m_adc_nb_channels; i++) {
		channel_sptr channel = std::make_shared<SpectrumChannel>(i, channel_names[i], fft_plot);
		...
		connect(channel.get()->widget(), SIGNAL(menuToggled(bool)), SLOT(onChannelSettingsToggled(bool)));
		connect(channel.get()->widget(), SIGNAL(selected(bool)), SLOT(onChannelSelected(bool)));
		connect(channel.get()->widget(), SIGNAL(enabled(bool)), SLOT(onChannelEnabled(bool)));
	...
	if(m_adc_nb_channels > 0)
		channels[crt_channel_id]->widget()->nameButton()->setChecked(true);
	...
	marker_freq_pos = new PositionSpinButton({{tr("Hz"), 1e0}, {tr("kHz"), 1e3}, {tr("MHz"), 1e6}},
	...
	connect(startStopRange, &StartStopRangeWidget::rangeChanged, [=](double start, double stop) {
		...
		for(; i < bin_sizes.size(); i++) {
			ui->cmb_rbw->addItem(freq_formatter.format(sample_rate / bin_sizes[i], "Hz", 2));
		}
		...
		marker_freq_pos->setStep(2 * (stop - start) / bin_sizes[ui->cmb_rbw->currentIndex()]);

		fft_plot->updateZoomerBase();
		waterfall_plot->updateZoomerBase();
	});

	connect(ui->cmb_rbw, QOverload<int>::of(&QComboBox::currentIndexChanged), [=](int index) {
	...
	connect(ui->cmbGainMode, QOverload<int>::of(&QComboBox::currentIndexChanged), [=](int index) {
		...
		try {
			libm2k::analog::ANALOG_IN_CHANNEL chn =
				static_cast<libm2k::analog::ANALOG_IN_CHANNEL>(crt_channel);
	...
	connect(marker_selector, SIGNAL(buttonToggled(int, bool)), this, SLOT(onMarkerToggled(int, bool)));
	connect(marker_selector, SIGNAL(buttonSelected(int)), this, SLOT(onMarkerSelected(int)));
	marker_selector->setSelectedButton(0);

	connect(fft_plot, SIGNAL(newMarkerData()), this, SLOT(onPlotNewMarkerData()));
	connect(fft_plot, SIGNAL(markerSelected(uint, uint)), this, SLOT(onPlotMarkerSelected(uint, uint)));

	connect(marker_freq_pos, SIGNAL(valueChanged(double)), this, SLOT(onMarkerFreqPosChanged(double)));

	connect(fft_plot, SIGNAL(sampleRateUpdated(double)), this, SLOT(onPlotSampleRateUpdated(double)));
	connect(fft_plot, SIGNAL(sampleCountUpdated(uint)), this, SLOT(onPlotSampleCountUpdated(uint)));

	connect(this, SIGNAL(selectedChannelChanged(int)), fft_plot, SLOT(setSelectedChannel(int)));
	connect(this, SIGNAL(selectedChannelChanged(int)), waterfall_plot, SLOT(enableChannel(int)));
	...
	connect(ui->runSingleWidget, &RunSingleWidget::toggled, this, &SpectrumAnalyzer::runStopToggled);
	connect(this, &SpectrumAnalyzer::started, ui->runSingleWidget, &RunSingleWidget::toggle);
	...
	connect(waterfall_size, SIGNAL(valueChanged(double)), SLOT(onWaterfallSizeChanged(double)));
	connect(top, SIGNAL(valueChanged(double)), SLOT(onTopValueChanged(double)));
	connect(unit_per_div, SIGNAL(valueChanged(double)), SLOT(onScalePerDivValueChanged(double)));
	connect(bottom, SIGNAL(valueChanged(double)), SLOT(onBottomValueChanged(double)));
	connect(top_scale, SIGNAL(valueChanged(double)), SLOT(onTopValueChanged(double)));
	connect(bottom_scale, SIGNAL(valueChanged(double)), SLOT(onBottomValueChanged(double)));

	cursor_panel_init();

	connect(fft_plot, SIGNAL(cursorReadoutsChanged(struct cursorReadoutsText)),
		SLOT(onCursorReadoutsChanged(struct cursorReadoutsText)));

	connect(waterfall_plot, SIGNAL(cursorReadoutsChanged(struct cursorReadoutsText)),
		SLOT(onCursorReadoutsChanged(struct cursorReadoutsText)));
	...
	connect(waterfallDocker, &QDockWidget::topLevelChanged, this, [=](bool floating) {
		if(!floating) {
			...
			if(flow != waterfall_plot->getFlowDirection())
				waterfall_plot->setFlowDirection(flow);
		}
	...
	marker_freq_pos->setStep(2 * (startStopRange->getStopValue() - startStopRange->getStartValue()) /
	...
	connect(ui->logBtn, &QPushButton::toggled, fft_plot, &FftDisplayPlot::useLogFreq);
	connect(ui->logBtn, &QPushButton::toggled, waterfall_plot, &WaterfallDisplayPlot::useLogFreq);
	connect(ui->logBtn, &QPushButton::toggled,
		[=](bool use_log_freq) { startStopRange->setMinimumValue(use_log_freq); });
	...
	connect(ui->btnSnapshot, &QPushButton::clicked, [=]() {
		...
		if(selected_ch_settings < 2) {
			...
			auto iterWin = std::find_if(
				win_types.begin(), win_types.end(),
				[&](const std::pair<QString, FftWinType> &p) { return p.second == sc->fftWindow(); });
			...
			if(!fileName.isEmpty()) {
				...
				fm.save(mag_data,
					"REF" + QString::number(selected_ch_settings - m_adc_nb_channels + 1) + "(" +
						unit + ")");

				QString channelDetails;

				auto iterAvg =
					std::find_if(avg_types.begin(), avg_types.end(),
						     [&](const std::pair<QString, FftDisplayPlot::AverageType> &p) {
							     return p.first ==
								     importedChannelDetails[selected_ch_settings -
											    m_adc_nb_channels][0];
						     });
				...
				auto iterWin =
					std::find_if(win_types.begin(), win_types.end(),
...
void SpectrumAnalyzer::readPreferences()
{
	...
	bool spectrumVisiblePeakSearch = p->get("M2k_spectrum_visible_peak_search").toBool();

	fft_plot->setVisibleFpsLabel(showFps);
	waterfall_plot->setVisibleFpsLabel(showFps);
	fft_plot->setVisiblePeakSearch(spectrumVisiblePeakSearch);
...
void SpectrumAnalyzer::btnExportClicked()
{
	...
	if(!fileName.isEmpty()) {
		...
		for(int i = 0; i < channels.size(); ++i) {
			...
			QString unit = ui->lblMagUnit->text();
			fm.save(data, "Amplitude CH" + QString::number(i + 1) + "(" + unit + ")");
...
{
	fft_plot->setHorizCursorsEnabled(on ? cr_ui->vCursorsEnable->isChecked() : false);
	fft_plot->setVertCursorsEnabled(on ? cr_ui->hCursorsEnable->isChecked() : false);

	fft_plot->trackModeEnabled(on ? cr_ui->btnNormalTrack->isChecked() : true);

	waterfall_plot->setHorizCursorsEnabled(on ? cr_ui->vCursorsEnable->isChecked() : false);
	waterfall_plot->setVertCursorsEnabled(on ? cr_ui->hCursorsEnable->isChecked() : false);
...
void SpectrumAnalyzer::on_btnSettings_clicked(bool checked)
{
	...
	if(checked && !menuOrder.isEmpty()) {
		...
		btn = static_cast<CustomPushButton *>(settings_group->checkedButton());
	}
...
void SpectrumAnalyzer::measure_settings_init()
{
	...
	connect(fft_plot, SIGNAL(channelAdded(int)), measure_settings, SLOT(onChannelAdded(int)));

	connect(this, SIGNAL(selectedChannelChanged(int)), measure_settings, SLOT(setSelectedChannel(int)));

	connect(ui->boxMeasure, SIGNAL(toggled(bool)), SLOT(setMeasuremensEnabled(bool)));
}
...
void SpectrumAnalyzer::onChannelAdded(int chnIdx)
{
	...
	if(fft_plot->isReferenceWaveform(chnIdx)) {
		...
		measure = new Measure(chnIdx, data, curve_size, nullptr, false);
	} else {
		...
		measure = new Measure(chnIdx, data, numPoints, nullptr, false);
	}
...
void SpectrumAnalyzer::measure_panel_init()
{
	...
	connect(this, SIGNAL(measurementsAvailable()), SLOT(onMeasuremetsAvailable()));
}

void SpectrumAnalyzer::init_selected_measurements(int chnIdx, std::vector<int> measureIdx)
{
...
void SpectrumAnalyzer::onMeasurementSelectionListChanged()
{
	...
	for(int i = 0; i < newList.size(); i++) {
		auto pMeasurement = measurement(newList[i].id(), newList[i].channel_id());
		if(pMeasurement) {
...
void SpectrumAnalyzer::measureCreateAndAppendGuiFrom(const MeasurementData &measurement)
{
...
void SpectrumAnalyzer::onNewDataReceived()
{
	...
	for(int i = 0; i < d_measureObjs.size(); i++) {
		...
		if(fft_plot->isReferenceWaveform(chn)) {
			...
			measure->setDataSource(fft_plot->getRef_data()[ref_idx], curve_size / 2);
			ref_idx++;
		} else {
			int64_t numPoints = fft_plot->getNumPoints() / 2;
			std::vector<double *> data = fft_plot->getOrginal_data();
...
void SpectrumAnalyzer::cursor_panel_init()
{
	...
	connect(cr_ui->btnLockHorizontal, &QPushButton::toggled, fft_plot, &FftDisplayPlot::setHorizCursorsLocked);
	connect(cr_ui->btnLockVertical, &QPushButton::toggled, fft_plot, &FftDisplayPlot::setVertCursorsLocked);

	connect(cr_ui->btnLockHorizontal, &QPushButton::toggled, waterfall_plot,
		&WaterfallDisplayPlot::setHorizCursorsLocked);
	connect(cr_ui->btnLockVertical, &QPushButton::toggled, waterfall_plot,
		&WaterfallDisplayPlot::setVertCursorsLocked);

	cursorsPositionButton = new CustomPlotPositionButton(cr_ui->posSelect);

	connect(cr_ui->hCursorsEnable, SIGNAL(toggled(bool)), fft_plot, SLOT(setVertCursorsEnabled(bool)));
	connect(cr_ui->vCursorsEnable, SIGNAL(toggled(bool)), fft_plot, SLOT(setHorizCursorsEnabled(bool)));
	connect(cr_ui->hCursorsEnable, SIGNAL(toggled(bool)), waterfall_plot, SLOT(setVertCursorsEnabled(bool)));
	connect(cr_ui->vCursorsEnable, SIGNAL(toggled(bool)), waterfall_plot, SLOT(setHorizCursorsEnabled(bool)));

	connect(cr_ui->btnNormalTrack, &QPushButton::toggled, this, &SpectrumAnalyzer::toggleCursorsMode);
...
void SpectrumAnalyzer::connectZoomers()
{
	...
	connect(fft_plot->getZoomer(), &QwtPlotZoomer::zoomed, this, [=](const QRectF &rect) {
		...
		auto const new_rect = QRectF(rect.left(), waterfall_zoomer->zoomBase().top(), rect.width(),
					     waterfall_zoomer->zoomBase().height());
	...
	connect(waterfall_plot->getZoomer(), &QwtPlotZoomer::zoomed, this, [=](const QRectF &rect) {
		...
		auto const new_rect = QRectF(rect.left(), fft_zoomer->zoomBase().top(), rect.width(),
		...
		if(fft_zoomer->zoomRectIndex() > waterfall_plot->getZoomer()->zoomRectIndex()) {
			...
			waterfall_plot->getZoomer()->setZoomStack(wf_stack,
								  waterfall_plot->getZoomer()->zoomRectIndex());
...
{
	if(!btnSyncPlotCursors)
		return;

	connect(btnSyncPlotCursors, &QPushButton::toggled, this, [=](bool toggled) { updateVisibleCursorHandles(); });
...
void SpectrumAnalyzer::add_ref_waveform(QVector<double> xData, QVector<double> yData)
{
	...
	channelWidget->deleteButton()->setProperty("curve_name", QVariant(qname));
	...
	connect(channelWidget, &ChannelWidget::menuToggled, this, &SpectrumAnalyzer::onChannelSettingsToggled);
	connect(channelWidget, &ChannelWidget::selected, this, &SpectrumAnalyzer::onChannelSelected);
	connect(channelWidget, &ChannelWidget::deleteClicked, this, &SpectrumAnalyzer::onReferenceChannelDeleted);
	connect(channelWidget, &ChannelWidget::enabled, this, &SpectrumAnalyzer::onChannelEnabled);
...
void SpectrumAnalyzer::onReferenceChannelDeleted()
{
	...
	referenceChannels.erase(std::find(referenceChannels.begin(), referenceChannels.end(), channelWidget));

	/* Update the id of the remaining channels */
	int id = m_adc_nb_channels;
	for(auto iter = referenceChannels.begin(); iter != referenceChannels.end(); ++iter) {
		(*iter)->setId(id++);
	}
	...
	if(ui->btnMarkers->isChecked() && !channelsEnabled) {
		...
		menuButtonActions.removeAll(QPair<CustomPushButton *, bool>(ui->btnMarkers, true));
...
void SpectrumAnalyzer::build_gnuradio_block_chain()
{
	...
	waterfall_sink = scopy::waterfall_sink::make(fft_size, window, 0, m_max_sample_rate, "Waterfall Plot",
						     m_adc_nb_channels, waterfall_plot, !use_float_sink);

	fft_sink = scopy::scope_sink_f::make(fft_size, m_max_sample_rate, "Osc Frequency", m_adc_nb_channels,
					     (QObject *)fft_plot);
	...
	for(size_t i = 0; i < m_adc_nb_channels; i++) {
		auto fft = gnuradio::get_initial_sptr(new fft_block(!use_float_sink, fft_size));
...
void SpectrumAnalyzer::build_gnuradio_block_chain_no_ctx()
{
	...
	auto window = SpectrumChannel::build_win(FftWinType::HAMMING, fft_size);
	waterfall_sink = scopy::waterfall_sink::make(fft_size, window, 0, m_max_sample_rate, "Waterfall Plot",
						     m_adc_nb_channels, waterfall_plot);
	...
	for(size_t i = 0; i < m_adc_nb_channels; i++) {
		auto fft = gnuradio::get_initial_sptr(new fft_block(false, fft_size));
		auto ctm = gr::blocks::complex_to_mag_squared::make(1);

		auto siggen =
			gr::analog::sig_source_f::make(m_max_sample_rate, gr::analog::GR_SIN_WAVE, 5e6 + i * 5e6, 2048);
		auto noise = gr::analog::fastnoise_source_f::make(gr::analog::GR_GAUSSIAN, 1, 0, 8192);
...
void SpectrumAnalyzer::on_comboBox_type_currentIndexChanged(const QString &s)
{
	...
	if(crt_channel < 0) {
		qDebug(CAT_M2K_SPECTRUM_ANALYZER) << "invalid channel ID for the opened Settings menu";
		return;
	}
...
void SpectrumAnalyzer::onChannelSettingsToggled(bool en)
{
	...
	triggerRightMenuToggle(static_cast<CustomPushButton *>(cw->menuButton()), en);
}
...
void SpectrumAnalyzer::updateChannelSettingsPanel(unsigned int id)
{
	...
	if(id < m_adc_nb_channels) {
		...
		auto it2 =
			std::find_if(win_types.begin(), win_types.end(), [&](const std::pair<QString, FftWinType> &p) {
...
ChannelWidget *SpectrumAnalyzer::getChannelWidgetAt(unsigned int id)
{
	...
	auto cwIter = std::find_if(
		referenceChannels.begin(), referenceChannels.end(),
		[&id](ChannelWidget *referenceChannelWidget) { return referenceChannelWidget->id() == id; });
...
void SpectrumAnalyzer::onChannelSelected(bool en)
{
	...
	if(marker_menu_opened) {
		triggerRightMenuToggle(static_cast<CustomPushButton *>(ui->btnMarkers), en);
	}
...
void SpectrumAnalyzer::updateMarkerMenu(unsigned int id)
{
	...
	if(cw->nameButton()->isChecked()) {
		...
		QString stylesheet = QString("border: 2px solid %1").arg(cw->color().name());
		...
		for(size_t i = 0; i < fft_plot->markerCount(id); i++) {
			marker_selector->setButtonChecked(i, fft_plot->markerEnabled(id, i));
		}
...
void SpectrumAnalyzer::onChannelEnabled(bool en)
{
	...
	if(en) {
		...
		if(allDisabled) {
			...
			ui->btnMarkers->setChecked(false);
			ui->btnMarkers->setDisabled(true);
			menuButtonActions.removeAll(QPair<CustomPushButton *, bool>(ui->btnMarkers, false));
			menuOrder.removeAll(ui->btnMarkers);
		}
...
void SpectrumAnalyzer::writeAllSettingsToHardware()
{
	try {
		if(m_m2k_analogin) {
			...
			for(unsigned int i = 0; i < m_adc_nb_channels; i++) {
				libm2k::analog::ANALOG_IN_CHANNEL chn =
					static_cast<libm2k::analog::ANALOG_IN_CHANNEL>(i);
...
void SpectrumAnalyzer::on_cmb_rbw_currentIndexChanged(int index)
{
	double samplingTime = sample_rate * fft_size;
	Preferences *p = Preferences::GetInstance();
	double target_fps = p->get("general_plot_target_fps").toDouble();
	double update_time = 1.0 / target_fps;
...
void SpectrumAnalyzer::setFftSize(uint size)
{
	...
	for(int i = 0; i < channels.size(); i++) {
		auto fft = gnuradio::get_initial_sptr(new fft_block(!use_float_sink, size));
		...
		if(use_float_sink) {
			...
			waterfall_ids[i] =
				iio->connect(waterfall_sink, i, i, use_float_sink, fft_size * m_nb_overlapping_avg);
		}
	...
	if(on) {
		...
		if(!mkExists) {
			ui->markerTable->addMarker(id, crt_channel_id, QString("M%1").arg(id + 1),
						   ...
						   fft_plot->markerMagnitude(crt_channel_id, id), markerTypes[mkType]);
		} else {
			ui->markerTable->updateMarker(id, crt_channel_id, fft_plot->markerFrequency(crt_channel_id, id),
...
{
	QString txtFreq = freq_formatter.format(fft_plot->markerFrequency(chIdx, mkIdx), "Hz", 3);
	QString txtMag = QString::number(fft_plot->markerMagnitude(chIdx, mkIdx), 'f', 3) +
		QString(" " + ui->cmb_units->currentText());
	QString txt = QString("Marker %1 -> ").arg(mkIdx + 1) + txtFreq + QString(" ") + txtMag;
	...
	for(size_t c = 0; c < m_adc_nb_channels; c++) {
		for(size_t m = 0; m < fft_plot->markerCount(c); m++) {
			if(fft_plot->markerEnabled(c, m)) {
				...
				ui->markerTable->updateMarker(m, c, fft_plot->markerFrequency(c, m),
							      fft_plot->markerMagnitude(c, m), markerTypes[mkType]);
			}
		}
	}
}
...
void SpectrumAnalyzer::onMarkerFreqPosChanged(double freq)
{
	...
	fft_plot->updateMarkerUi(crt_channel_id, crt_marker);
	double actual_freq = fft_plot->markerFrequency(crt_channel_id, crt_marker);
...
{
	marker_freq_pos->setMaxValue(startStopRange->getStopValue());
	marker_freq_pos->setStep(2 * startStopRange->getStopValue() / bin_sizes[ui->cmb_rbw->currentIndex()]);
...
QPair<int, int> SpectrumAnalyzer::getGridLayoutPosFromIndex(QGridLayout *layout, int index) const
{
...
void SpectrumAnalyzer::setCurrentAverageIndexLabel(uint chnIdx)
{
	if(chnIdx == crt_channel_id) {
		if(channels[chnIdx]->averaging() != 1) {
			...
			QString txt = QString(tr("Average Sample:") + " %1 / %2 ")
					      .arg(channels[chnIdx]->averageIdx())
 ...
 */
SpectrumChannel::SpectrumChannel(int id, const QString &name, FftDisplayPlot *plot)
	: m_id(id)
	, m_name(name)
	, m_line_width(1.0)
...
float SpectrumChannel::calcCoherentPowerGain(const std::vector<float> &win) const
{
	float sum = 0;
