...
ClockPatternUI::ClockPatternUI(ClockPattern *pattern, QWidget *parent)
	: PatternUI(parent)
	, pattern(pattern)
	, parent_(parent)
{
	...
	ui->setupUi(this);
	frequencySpinButton = new ScaleSpinButton({{"Hz", 1E0}, {"kHz", 1E+3}, {"MHz", 1E+6}}, tr("Frequency"), 1e0,
						  PG_MAX_SAMPLERATE / 2, true, false, this, {1, 2.5, 5});
	ui->verticalLayout->addWidget(frequencySpinButton);
	phaseSpinButton = new PhaseSpinButton({{"deg", 1}, {"π rad", 180}}, tr("Phase"), 0, 360, true, true, this);
...
NumberPatternUI::NumberPatternUI(NumberPattern *pattern, QWidget *parent)
	: PatternUI(parent)
	, pattern(pattern)
	, parent_(parent)
	, max(0)
...
uint16_t UARTPattern::encapsulateUartFrame(char chr, uint16_t *bits_per_frame)
{
	...
	if(!msb_first) {
		...
		/*      if(parity_bit_available)
		{
		    ...
		    stop_bit_values = ((1<<stop_bits)-1); // if parity bit not availabe, stop bits will not be
		incremented
		*/
...
UARTPatternUI::UARTPatternUI(UARTPattern *pattern, QWidget *parent)
	: PatternUI(parent)
	, pattern(pattern)
	, parent_(parent)
{
	...
	connect(this, &UARTPatternUI::patternParamsChanged, [=]() {
		...
		m_decoder->set_option("baudrate", g_variant_new_int64(pattern->get_baud_rate()));
		m_decoder->set_option("data_bits", g_variant_new_int64(8));
		...
		m_decoder->set_option("stop_bits", g_variant_new_double(pattern->get_stop_bits()));
...
I2CPatternUI::I2CPatternUI(I2CPattern *pattern, QWidget *parent)
	: PatternUI(parent)
	, pattern(pattern)
	, parent_(parent)
...
SPIPatternUI::SPIPatternUI(SPIPattern *pattern, QWidget *parent)
	: PatternUI(parent)
	, pattern(pattern)
	, parent_(parent)
{
...
JSPattern::JSPattern(QJsonObject obj_)
	: obj(obj_)
{
...
uint32_t JSPattern::get_min_sampling_freq()
{
	...
	if(result.isNumber()) {
		...
		qDebug() << "Error - return value of get_min_sampling_freq() is a string - " << result.toString();
	} else {
...
uint32_t JSPattern::get_required_nr_of_samples()
{
	...
	if(result.isNumber()) {
		...
		qDebug() << "Error - return value of get_required_nr_of_samples() is a string - " << result.toString();
	} else {
...
bool JSPattern::is_periodic()
{
	...
	if(result.isBool()) {
		...
		qDebug() << "Error - return value of is_periodic() is a string - " << result.toString();
	} else {
...
uint8_t JSPattern::pre_generate()
{
	...
	qEngine->evaluate("function get_required_nr_of_samples(){ status_window.print(\"get_required_nr_of_samples() "
			  "not found\")}");
	qEngine->evaluate(
		"function get_min_sampling_freq(){ status_window.print(\"get_min_sampling_freq() not found\")}");
...
JSPatternUI::JSPatternUI(JSPattern *pat, QJsonObject obj_, QWidget *parent)
	: pattern(pat)
	, parent_(parent)
	, jspat_api(nullptr)
	, PatternUI(parent)
...
void JSPatternUI::post_load_ui()
{
	...
	QJSValue uiObj = pattern->qEngine->newQObject(/*ui_form*/ ui->ui_form->findChild<QWidget *>(form_name));
...
uint32_t PulsePattern::get_required_nr_of_samples(uint32_t sample_rate, uint32_t number_of_channels)
{
...
PulsePatternUI::PulsePatternUI(PulsePattern *pattern, QWidget *parent)
	: PatternUI(parent)
	, pattern(pattern)
	, parent_(parent)
{
	...
	ui = new Ui::PulsePatternUI();
	ui->setupUi(this);
	frequencySpinButton = new ScaleSpinButton({{"Hz", 1E0}, {"kHz", 1E+3}, {"MHz", 1E+6}}, tr("Sample Rate"), 1e0,
						  PG_MAX_SAMPLERATE, true, false, this, {1, 2.5, 5});
