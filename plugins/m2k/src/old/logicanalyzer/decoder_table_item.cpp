...
void DecoderTableItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option,
...
QSize DecoderTableItemDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
...
void DecoderTableItem::paint(QPainter *painter, const QRect &rect, const QPalette &palette) const
{
	...
	for(int row = 0; row < curve->getAnnotationRows().size(); ++row) {
		...
		if(filteredMessages.contains(curve->fromTitleToRowType(entry.first.title())))
			continue;
		if(data.size() == 0)
			continue;
		...
		for(const auto &ann : range) {
			curve->drawAnnotation(offset, ann, painter, xmap, ymap, rect, mapper, interval, titleSize);
		}
		offset += 1;
	}
