...
void DecoderTableModel::setPrimaryAnnotation(int index)
{
	...
	if(m_decoderTable->isActive() &&
	   m_curves.at(m_current_column)->getMaxAnnotationCount(index) !=
		   m_curves.at(m_current_column)->getMaxAnnotationCount(prev_index)) {
		refreshColumn(m_current_column);
	}

	int max_value =
		m_curves.at(m_current_column)->getMaxAnnotationCount(m_primary_annoations->value(m_current_column));
...
void DecoderTableModel::setCurrentRow(int index)
{
	...
	if(index >= curve->getMaxAnnotationCount(m_primary_annoations->value((int)m_current_column)))
		return;
...
void DecoderTableModel::refreshColumn(double column) const
{
	...
	bool no_rows = true;
	int grouped_rows =
		(index - m_logic->getGroupOffset()) / m_logic->getGroupSize() + (m_logic->getGroupOffset() != 0);
...
void DecoderTableModel::populateFilter(int index) const
{
	...
	for(int row = 0; row < temp_curve->getAnnotationRows().size(); ++row) {
		auto row_map = *std::find_if(
			temp_curve->getAnnotationRows().begin(), temp_curve->getAnnotationRows().end(),
...
{
	if(!index.isValid() || m_decoderTable->signalsBlocked() || m_plotCurves.empty())
		return QVariant();

	const size_t col = index.column();
	if(col >= m_curves.size())
		return QVariant();
	...
	if(to_be_refreshed) {
		refreshColumn(col);
		to_be_refreshed = false;
		if(to_be_refreshed == 2)
			selectedDecoderChanged(col);
	}

	int64_t total_rows = (m_logic->getGroupOffset() == 0)
		? (row.size() - m_logic->getGroupOffset()) / m_logic->getGroupSize()
	...
	return QVariant::fromValue(DecoderTableItem(temp_curve, row[start_ann].start_sample(),
						    row[end_ann].end_sample(), m_filteredMessages.value(index.column()),
	...
	if(!searchString.isEmpty()) {
		...
		while(i < row.size()) {
			...
			if(i == 0 && m_logic->getGroupOffset() != 0) {
				...
				end_sample = row[std::min(uint64_t(i + m_logic->getGroupSize() - 1),
							  uint64_t(row.size() - 1))]
						     .end_sample();
			}
			row_index++;
			...
			for(const auto &row_map : decoder) {
				...
				if(m_filteredMessages.value(m_current_column).contains(title))
					continue;
				...
				while(j <= index_range.second && j < row_map.second.size()) {
					...
					if((title != primary_title &&
					    (((unsigned)(ann.end_sample() - start_sample - 1) <=
						      (end_sample - start_sample - 1) ||
					      (unsigned)(ann.start_sample() - start_sample) <
