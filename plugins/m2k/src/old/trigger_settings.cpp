...
{
	externalTriggerOutMapping = {{tr("Forward Trigger In"), libm2k::SELECT_TRIGGER_IN},
				     {tr("Oscilloscope"), libm2k::SELECT_ANALOG_IN},
				     {tr("Logic Analyzer"), libm2k::SELECT_DIGITAL_IN}};
}

TriggerSettings::TriggerSettings(M2kAnalogIn *libm2k_adc, QWidget *parent)
	: QWidget(parent)
	...
	, digital_trigger_was_on(false)
{
	...
	trigger_level = new PositionSpinButton({{tr("μVolts"), 1e-6}, {tr("mVolts"), 1e-3}, {tr("Volts"), 1e0}},
					       tr("Level"), 0.0, 0.0, true, false, this);

	trigger_hysteresis = new PositionSpinButton({{tr("μVolts"), 1e-6}, {tr("mVolts"), 1e-3}, {tr("Volts"), 1e0}},
	...
	connect(ui->btnLogicAnalyzerTriggers, &QPushButton::clicked, this,
		&TriggerSettings::showLogicAnalyzerTriggerSettings);
}
...
void TriggerSettings::enableExternalTriggerOut(bool enabled)
{
	if(m_has_external_trigger_out) {
		try {
			if(enabled) {
				m_trigger->setAnalogExternalOutSelect(
...
void TriggerSettings::setTriggerLevelRange(int chn, const std::pair<double, double> &range)
{
...
void TriggerSettings::setTriggerHystRange(int chn, const std::pair<double, double> &range)
{
...
{
	if(adc_running) {
		libm2k::M2K_TRIGGER_CONDITION_ANALOG t_cond = static_cast<libm2k::M2K_TRIGGER_CONDITION_ANALOG>(cond);
...
{
	if(adc_running) {
		libm2k::M2K_TRIGGER_CONDITION_DIGITAL t_cond = static_cast<libm2k::M2K_TRIGGER_CONDITION_DIGITAL>(cond);
...
void TriggerSettings::writeHwMode(int mode)
{
	if(adc_running) {
		try {
			m_trigger->setAnalogMode(currentChannel(), static_cast<libm2k::M2K_TRIGGER_MODE>(mode));
		} catch(libm2k::m2k_exception &e) {
			HANDLE_EXCEPTION(e);
			qDebug() << e.what();
		}
...
void TriggerSettings::writeHwSource(int source_chn)
{
	if(adc_running) {
		try {
			...
			bool extern_digital_checked =
				(ui->extern_en->isChecked() && ui->cmb_extern_src->currentIndex() == 1);
			if(m_trigger_in) {
				if(!intern_checked) {
