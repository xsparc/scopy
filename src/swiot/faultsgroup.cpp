...
adiscope::gui::FaultsGroup::FaultsGroup(QString name, const QString& path, QWidget *parent) :
	QWidget(parent),
	...
	m_customColGrid(new CustomColQGridLayout(MAX_COLS_IN_GRID, false, this))
{
	...
	if (document.isNull()) { qCritical(CAT_NEWINSTRUMENT) << "Invalid json: " << parse_error.errorString(); }

	QJsonObject document_object = document.object();
	QJsonValue device_value = document_object.value(m_name);
	if (device_value == QJsonValue::Undefined) { qCritical(CAT_NEWINSTRUMENT) << "Invalid json: Could not extract value " << m_name; }
	...
	for (int i = 0; i < m_max_faults; ++i) {
		...
		connect(fault_widget, &FaultWidget::faultSelected, this, [=](unsigned int id_){
	...
	connect(m_customColGrid, &CustomColQGridLayout::reqestLayoutUpdate, this, [=](){
...
void adiscope::gui::FaultsGroup::setupDynamicUi() {
...
void adiscope::gui::FaultsGroup::clearSelection() {
...
void adiscope::gui::FaultsGroup::update(uint32_t faults_numeric) {
...
QString adiscope::gui::FaultsGroup::getExplanations() {
	QString res = "";
	if (!m_currentlySelected.empty()) {
		for (unsigned int i : m_currentlySelected) {
			if (m_faults.at(i)->isStored()) {
				res += QString("Bit%1 (%2): %3\n").arg(QString::number(i), m_faults.at(i)->getName(), m_faults.at(i)->getFaultExplanation());
			}
		}
	} else {
...
QWidget *adiscope::gui::FaultsGroup::buildActiveStoredWidget() {
