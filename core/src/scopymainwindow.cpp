...
void ScopyMainWindow::loadOpenGL()
{
	...
	if (ct) {
		...
		qInfo(CAT_BENCHMARK) << "GL_RENDERER " << reinterpret_cast<const char*>(glFuncs->glGetString(GL_RENDERER));
		qInfo(CAT_BENCHMARK) << "GL_VERSION " << glVersion;
		qInfo(CAT_BENCHMARK) << "GL_EXTENSIONS " << reinterpret_cast<const char*>(glFuncs->glGetString(GL_EXTENSIONS));
	...
	qInfo(CAT_BENCHMARK) << "OpenGL load success status: " << !disablePref;
