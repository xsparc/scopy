...
public:
	...
	inline void store_first_closest_val_to_cross_lvl(const float *data, size_t i, size_t &point)
	{
		double diff1 = qAbs(data[i - 1] - m_level);
		double diff2 = qAbs(data[i] - m_level);
...
private:
	...
	void maskedMax(int &index, double &value, std::vector<int> mask, int start = 0, int end = 0)
	{
		...
		float *data_at_index = new float[end]();
	...
	void getMax(const float *data, int size, int &index, double &value)
	{
		index = 0;
		value = data[0];
...
void MeasureModel::setDataSource(const float *buffer, size_t length)
{
	m_buffer = buffer;
	m_buf_length = length;
}
...
void TimeMeasureModel::measure()
{
	...
	measureTime();
	Q_EMIT newMeasurementsAvailable();
}

void TimeMeasureModel::measureTime()
{
	double period;
	double frequency;
	double rise_time;
