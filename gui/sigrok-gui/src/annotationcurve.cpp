...
AnnotationCurve::AnnotationCurve(std::shared_ptr<logic::Decoder> initialDecoder)
	: GenericLogicPlotCurve(initialDecoder->decoder()->name, initialDecoder->decoder()->id,
				LogicPlotCurveType::Annotations)
	, m_visibleRows(0)
{
	setSamples(QVector<double>({0.0}), QVector<double>({0.0})), setRenderHint(RenderAntialiased, true);
				...
				const QRectF &canvasRect, int from, int to) const
{
	...
	for(size_t row = 0; row < m_annotationRows.size(); ++row) {
		auto it = std::find_if(
			m_annotationRows.begin(), m_annotationRows.end(),
		...
		const double min_ann_label_width =
			QFontMetrics(QFont("Times", 10, QFont::Bold)).horizontalAdvance("XX");

		for(; start <= stop; ++start) {
			Annotation ann = ((*it).second.getAnnAt(start));

			const double annotation_width = xMap.transform(fromSampleToTime(ann.end_sample())) -
				xMap.transform(fromSampleToTime(ann.start_sample()));
			...
			const double delta = (previous_end != -1 ? xMap.transform(fromSampleToTime(ann.end_sample())) -
							      xMap.transform(fromSampleToTime(previous_end))
								 : 0.0);

			if(qAbs(delta) > 1.5 || shouldDraw) {
				if(annotations_in_block == 1) {
					drawAnnotation(currentRowOnPlot, prev_ann, painter, xMap, yMap, canvasRect,
						       mapper, interval, size);
				} else if(annotations_in_block > 0) {
					drawBlock(currentRowOnPlot, block_start, previous_end, painter, xMap, yMap,
						  canvasRect, mapper);
				}
			...
			if(shouldDraw) {
				drawAnnotation(currentRowOnPlot, ann, painter, xMap, yMap, canvasRect, mapper, interval,
					       size);
				previous_end = -1;
				annotations_in_block = 0;
			} else {
	...
	for(const QString &title : titles) {
		...
		textRect.moveCenter(
			QPointF(xMap.transform(interval.minValue()), yMap.transform(offset + HeightInPoints / 2.0)));

		textRect.setBottomLeft(textRect.bottomLeft() +
				       QPointF(size.width() / 2.0 + pixOffsetFromCanvasMargin, 0));
...
void AnnotationCurve::drawBlock(int row, uint64_t start, uint64_t end, QPainter *painter, const QwtScaleMap &xMap,
				const QwtScaleMap &yMap, const QRectF &canvasRect, const QwtPointMapper &mapper) const
{
	...
	const QRectF rect(QPointF(xMap.transform(fromSampleToTime(start)), yMap.transform(offset)),
			  QSizeF(width, m_traceHeight));
					 ...
					 const QRectF &canvasRect) const
{
	...
	QRectF rect;
	if(start != end) {
		rect = QRectF(QPointF(xMap.transform(fromSampleToTime(start)), yMap.transform(offset)),
			      QSizeF(width, m_infoHeight));
	} else {
		rect = QRectF(QPointF(canvasRect.x() + canvasRect.width() * infoWidthRatio, yMap.transform(offset)),
			      QSizeF(canvasRect.width() * (1 - infoWidthRatio * 2), m_infoHeight));
	}
	...
	QString info_delta_sample =
		QString("sample count: %1  ").arg(QString::fromStdString(std::to_string(end - start)));
...
{
	return (title.indexOf(':')) ? title.mid(title.indexOf(':') + 1) : title;
}

void AnnotationCurve::drawAnnotation(int row, const Annotation &ann, QPainter *painter, const QwtScaleMap &xMap,
				     ...
				     const QwtInterval &interval, const QSizeF &titleSize) const
{
	...
	if(ann.start_sample() != ann.end_sample()) {
		drawTwoSampleAnnotation(row, ann, painter, xMap, yMap, canvasRect, mapper, interval, titleSize);
	} else {
		drawOneSampleAnnotation(row, ann, painter, xMap, yMap, canvasRect, mapper, interval, titleSize);
	}
}

void AnnotationCurve::fillAnnotationCurve(int row, uint32_t annClass, QPainter *painter, const QwtScaleMap &xMap,
					      ...
					      const QwtInterval &interval, const QSizeF &titleSize) const
{
	...
	// draw the text centered if the annotation goes beyond plot limits, make it fit the visible area (left/right
	// shift from center)
...
AnnotationQueryResult AnnotationCurve::annotationAt(const QPointF &p) const
{
	...
	const uint64_t offset_samples =
		std::max(fromTimeToSample(offset_time2) - fromTimeToSample(offset_time1), ulong(1));
