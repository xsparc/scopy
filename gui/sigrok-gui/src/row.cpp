...
const QString Row::title() const
{
	if(decoder_ && decoder_->name && row_ && row_->desc)
		return QString("%1: %2").arg(QString::fromUtf8(decoder_->name), QString::fromUtf8(row_->desc));
...
{
	return (decoder_ < other.decoder_) || (decoder_ == other.decoder_ && row_ < other.row_);
}
