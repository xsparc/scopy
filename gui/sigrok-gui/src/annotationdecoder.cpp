	...
	for(l = srd_d->channels; l; l = l->next) {
		...
		if(!ch_added) {
			...
			DecodeChannel ch = {id++,
					    ...
					    false,
					    false,
					    QString::fromUtf8(pdch->name),
					    QString::fromUtf8(pdch->desc),
...
AnnotationDecoder::AnnotationDecoder(AnnotationCurve *annotationCurve, std::shared_ptr<logic::Decoder> initialDecoder)
...
void AnnotationDecoder::dataAvailable(uint64_t from, uint64_t to, uint16_t *data)
{
...
void AnnotationDecoder::assignChannel(uint16_t chId, uint16_t bitId)
{
	...
	for(auto &ch : m_channels) {
		qDebug() << "ch: " << ch.id << " has ch assigned: " << ch.assigned_signal
			 << " with bitid: " << ch.bit_id;
	}
...
void AnnotationDecoder::stackChanged()
{
	...
	for(const shared_ptr<logic::Decoder> &dec : m_stack) {
		...
		for(const GSList *l = decc->annotation_rows; l; l = l->next) {
			const srd_decoder_annotation_row *const ann_row = (srd_decoder_annotation_row *)l->data;
			assert(ann_row);
			...
			for(const GSList *ll = ann_row->ann_classes; ll; ll = ll->next) {
				m_class_rows[std::make_pair(decc, GPOINTER_TO_INT(ll->data))] = row;
				count++;
			}
		}
	}
	...
	srd_pd_output_callback_add(m_srdSession, SRD_OUTPUT_ANN, AnnotationCurve::annotationCallback,
				   m_annotationCurve);
}
...
void AnnotationDecoder::decodeProc()
{
	while(!m_decodeCanceled) {
		...
		if(srd_session_send(m_srdSession, start, stop, reinterpret_cast<uint8_t *>(chunk.get()), chunkSize,
				    sizeof(uint16_t)) != SRD_OK) {
			//            qDebug() << "No bueno!";
		}
