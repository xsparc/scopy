...
void MenuHeader::setEnabledBtnState(bool state)
{
	m_ui->btnEnabled->setChecked(state);
}

void MenuHeader::setLabel(const QString &text)
{
	m_ui->lblTitle->setText(text);
}
...
void MenuHeader::setEnableBtnVisible(bool visible)
{
	m_ui->btnEnabled->setVisible(visible);
}

QPushButton *MenuHeader::getEnableBtn()
{
	return m_ui->btnEnabled;
}
